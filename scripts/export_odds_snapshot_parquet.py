...
def main() -> None:
    args = parse_args()
    snapshot_time = _latest_snapshot_time(args.market_db)
    if snapshot_time is None:
        raise SystemExit(f"No odds snapshots found in {args.market_db}")
    rows = _snapshot_rows(args.market_db, snapshot_time)
    if not rows:
        raise SystemExit(f"No odds rows found for snapshot_time={snapshot_time}")

    parsed_snapshot_time = _parse_time(snapshot_time)
    ...
    _write_parquet(rows, output_path, snapshot_time=parsed_snapshot_time)

    s3_key = "/".join(output_path.relative_to(args.output_dir).parts)
    print(f"Exported {len(rows)} odds rows to {output_path}")
    print(f"S3 key: {s3_key}")
    if args.github_output:
        args.github_output.write_text(
            f"local_path={output_path}\n"
            f"s3_key={s3_key}\n"
            f"snapshot_time={parsed_snapshot_time.isoformat()}\n",
            encoding="utf-8",
        )
...
def _latest_snapshot_time(db_path: Path) -> str | None:
...
def _snapshot_rows(db_path: Path, snapshot_time: str) -> list[dict[str, Any]]:
    with _connect(db_path) as db:
        rows = db.execute(
            """
            SELECT
    ...
    return [_parquet_row(row) for row in rows]


def _parquet_row(row: sqlite3.Row) -> dict[str, Any]:
    ...
    return {
        ...
        "point": _point_value(row["point_key"]),
        ...
        "last_update": _parse_time(row["last_update"]),
...
def _write_parquet(
    ...
    *,
    ...
    try:
        ...
        raise SystemExit("Missing pyarrow. Install it with: pip install pyarrow") from exc
...
def _output_path(output_dir: Path, *, s3_prefix: str, snapshot_time: datetime) -> Path:
...
def _point_value(value: str) -> float | None:
